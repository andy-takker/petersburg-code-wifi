...
from controllers.repo.wifi_zone import WifiZoneRepo
...
async def get_wifi_zones(limit: int, offset: int,
    ...
    return await wifi_zone.get_all_wifi_zones(limit=limit, offset=offset)
...
async def get_nearest_wifi_zones(limit: int, offset: int,
                                 ...
                                 radius: float = 2,
                                 wifi_zone: WifiZoneRepo = Depends()) -> Any:
    return await wifi_zone.get_nearest_wifi_zones(limit=limit, offset=offset,
                                                  lat_x=latitude,
                                                  lon_x=longitude,
                                                  radius=radius)
